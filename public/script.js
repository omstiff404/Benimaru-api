...
(function initProfileMedia() {
  ...
  // spectrum lebar di beranda
  try {
    const w = Math.min(640, (canvas.parentElement?.clientWidth || 320) * 2);
    canvas.width = w || 640;
    canvas.height = 48;
  } catch {}
  ...
  async function play() {
    ...
    try {
      ...
      const iq = playBtn.querySelector('.icon-pause');
      if (ip) ip.hidden = true;
      if (iq) iq.hidden = false;
  ...
  function pause() {
    ...
    const ip2 = playBtn.querySelector('.icon-play');
    const iq2 = playBtn.querySelector('.icon-pause');
    if (ip2) ip2.hidden = false;
    if (iq2) iq2.hidden = true;
    cancelAnimationFrame(raf);
    drawIdle();
  }
  ...
  if (video) {
    ...
    let vi = 0;
    const profile = document.querySelector('.home-page') || document.querySelector('#homeStage');
    const tryVideo = () => {
